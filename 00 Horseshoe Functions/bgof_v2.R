  ...
  if (DN == FALSE) { # undirected
  	for (i in 1:sample.size) {
	  	a <- gof(x$formula,
	  	         coef = FF[i, ],
	  	         verbose = FALSE,
	  	         GOF =  .~ degree + espartners + distance + triadcensus + model,
  		...
  		if (i == 1) D <- as.vector(a$pobs.triadcensus)
  		D <- cbind(D,as.vector(a$psim.triadcensus[2,]))
  	}
  	if (is.null(n.deg)) n.deg <- dim(A)[1]
  	if (is.null(n.dist)) n.dist <- dim(B)[1] - 1
  	if (is.null(n.esp)) n.esp <- dim(C)[1]
  	if (is.null(n.triad)) n.triad <- dim(D)[1]
  	
  	if (names(a$pobs.deg)[1] == "degree0") {
  	  a5 <- apply(A[1:(n.deg+1), -1],
  	              1, quantile, probs = 0.05)
  	  a95 <- apply(A[1:(n.deg+1), -1],
  	...
  	c5 <- apply(C[1:n.esp, -1],
  	            1, quantile, probs = 0.05)
  	d5 <- apply(D[1:n.triad,-1],
  	            1, quantile, probs = 0.05)
  	b95 <- apply(B[-(n.dist:(dim(B)[1] - 1)), -1],
  	            1, quantile, probs = 0.95)
  	c95 <- apply(C[1:n.esp, -1],
  	            1, quantile, probs = 0.95)
  	d95 <- apply(D[1:n.triad,-1],
  	             1, quantile, probs = 0.95)
  	par(mfcol = c(2,2), oma = c(0, 0, 3, 0), mar = c(4,4.4,0.75,0.25))
  	
  	if (names(a$pobs.deg)[1] == "degree0") {
  	  boxplot(as.data.frame(t(A[1:(n.deg+1), -1])),
  	          xaxt = "n",
  	          xlab = "degree",
  	          ylab = "proportion of nodes", 
  	          at = 0:n.deg, ...)
  	  axis(1, c(seq(0, n.deg-3, by = 4),n.deg))
  	  lines(A[1:(n.deg+1), 1], x = 0:n.deg, lwd = 2, col = 2)
  	  lines(a5,x = 0:n.deg, col = "darkgray")
  	  lines(a95,x = 0:n.deg, col = "darkgray")
  	} else {
  	...
  	axis(1,c(seq(0, n.esp-3, by = 4),n.esp))
  	lines(C[1:n.esp, 1],lwd = 2,col = 2)
  	lines(c5,col = "darkgray")
  	lines(c95,col = "darkgray")
  	
  	boxplot(as.data.frame(t(D[1:n.triad,-1])),
  	        xaxt = "n",
  	        xlab = "triad census",
  	        ylab = "proportion of triads",
  	        at = 0:(n.triad-1),...)
  	axis(1,c(seq(0, n.triad-1, by = 1)))
  	lines(D[1:n.triad,1],x = 0:(n.triad-1),lwd = 2,col = 2)
  	lines(d5,x = 0:(n.triad-1), col = "darkgray")
  	lines(d95,x = 0:(n.triad-1), col = "darkgray")
  	...
  	par(mfrow = c(2, 2), oma = c(0, 0, 2.8, 0), mar = c(4.5,4.5,1,1))
  	
  	boxplot(as.data.frame(t(A[1:n.ideg,-1])),
  	        xaxt = "n",
  	        xlab = "in degree",
  	        ylab = "proportion of nodes",...)
  	axis(1, c(seq(0, n.ideg-3, by = 6),n.ideg))
  	...
  	boxplot(as.data.frame(t(AA[1:n.odeg, -1])),
  	        ...
  	        xlab = "out degree",
  	        ylab = "proportion of nodes",...)
  	axis(1,c(seq(0,n.odeg-3, by = 6),n.odeg))
